...
export function DrawerSnapPoints() {
  ...
  return (
    <>
      <div
        className={`${
          snap === 1 ? "translate-y-0" : "-translate-y-full"
        } transition-all h-20 z-50 fixed top-0 inset-x-0 max-w-md mx-auto bg-white shadow-md`}
      ></div>
      <Drawer.Root
        snapPoints={["300px", 1]}
      ...
      >
        ...
        <Drawer.Portal>
          <Drawer.Content className="max-w-md mx-auto fixed flex flex-col bg-white border border-gray-200 border-b-none rounded-t-[10px] bottom-0 left-0 right-0 h-full max-h-[94%]">
            <div className="mt-4">
              <DragButton />
            </div>
            <div
